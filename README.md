...
## ⚠️ AI Disclaimer & Safety Notice

This application compiles and runs arbitrary C++ code inside a sandboxed container.

- **Execution Risks**: Any code executed via the REPL or Standalone editor runs as a native binary within the Docker container. Always exercise caution when running code that interacts with the network, filesystem, or operating system.
- **AI-Generated Code Safety**: If you are using this playground to test code generated by artificial intelligence models (such as Gemini, ChatGPT, Claude, or Antigravity), **always review the code for correctness and safety before executing it**. AI-generated code can contain hidden logic bugs, security exploits, or infinite loops.
- **Resource Sandboxing**: To mitigate risks (like infinite loops, memory leaks, or fork bombs), the playground runs with built-in constraints:
  - **Memory Limit**: 256MB.
  - **CPU Allocation**: 0.5 cores.
  - **Execution Timeout**: 5 seconds (enforced via OS-level SIGKILL timeout).
